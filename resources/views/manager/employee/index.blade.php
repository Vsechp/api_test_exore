            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-1 mb-3">
                        <a href="{{route('manager.employee.create')}}" class="btn btn-block btn-outline-dark">Add</a>
                    </div>
                </div>
                            ...
                            <div class="card-body table-responsive p-0">
                                <table class="table table-hover text-nowrap">
                                    ...
                                    @foreach($employees as $employee)
                                        <tr>
                                            <td>{{$employee->id}}</td>
                                            <td><a href="{{route('manager.employee.show', $employee->id)}}">{{$employee->name}}</a></td>
                                            <td>{{$employee->role}}</td>
                                            <td>{{$employee->email}}</td>
                                            <td><a href="{{route('manager.employee.show', $employee->id)}}"><i class="far fa-eye"></i></a></td>
                                            <td><a href="{{route('manager.employee.edit', $employee->id)}}"
                                                   class="text-success"><i class="fas fa-pen"></i></a></td>
                                            <td>
                                                <form action="{{route('manager.employee.delete', $employee->id)}}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
